...
# 日志配置
log_file = "/opt/py_prod/log/special_user_allow.log"
...
for line in sys.stdin:
    try:
        ...
        if not r.exists(user_key):
            ...
            print("ERR message=no_special_group_matched")
            sys.stdout.flush()
            continue
        ...
        if not policy_raw:
            ...
            print("ERR message=no_policy_match")
            sys.stdout.flush()
            continue
        ...
        try:
            ...
            # trie = build_trie(allow_list)

            # if match_trie(trie, hostname):
            if check_access(hostname, allow_list):
                log_request(username, domain, f"allow_by:{username}")
                print("OK")
                sys.stdout.flush()
                continue
            else:
