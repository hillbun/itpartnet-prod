...
LOG_FILE = f"{LOG_DIR}iprange_allow.log"
log_lock = threading.Lock()

def log_request(ip_range, domain, result, debug_info=""):
    timestamp = datetime.datetime.now().isoformat()
    message = f"[{timestamp}] [IPRANGE-ALLOW] range={ip_range} domain={domain} result={result} {debug_info}\n"
    with log_lock:
    ...
    for line in sys.stdin:
        try:
            ...
            if not matched_range:
                log_request("N/A", f"{hostname}", "no_matching_range")
                print("ERR")
                continue
            ...
            # trie = build_trie(allow_rules)
            # matched, match_path = match_trie(trie, hostname)
            matched, match_path = check_access(hostname, allow_rules)
            
            if matched:
                debug_info = " | ".join(match_path)
                log_request(matched_range, f"{hostname}", "allow_matched", debug_info)
                print("OK")
            else:
                print("ERR")
