...
LOG_FILE = f"{LOG_DIR}ip_deny.log"
log_lock = threading.Lock()

def log_request(source, domain, result, debug_info=""):
    timestamp = datetime.datetime.now().isoformat()
    message = f"[{timestamp}] [IP-DENY] source={source} domain={domain} result={result} {debug_info}\n"
    with log_lock:
    ...
    for line in sys.stdin:
        try:
            ...
            if matched:
                ...
                log_request(ip, f"{hostname}", "deny_matched")
                print("OK")
            else:
                #debug_info = " | ".join(match_path)
                log_request(ip, f"{hostname}", "not_matched", )
                print("ERR")
